...
impl SerialPubChannels {
    ...
    pub fn iter(&self) -> impl Iterator<Item = &SerialChannelName> {
        self.channels.iter().map(|topic| topic)
    }

    pub fn into_iter(&self) -> impl Iterator<Item = SerialChannelName> {
        self.channels.clone().into_iter().map(|topic| topic)
    }
}
...
mod tests {
    ...
    fn test_add_channel_name() {
        ...
        assert!(channels.into_iter().any(|l| l == channel_name));
    }
    ...
    fn test_iter_channels() {
        ...
        let collected: Vec<_> = channels.into_iter().collect();
